...
void update_PID_params (const geometry_msgs::Twist& cmd_vel){
  linear =  cmd_vel.linear.x;
  angular = cmd_vel.angular.z;
//linear higher than 0.5 will be high speed


if(linear > 0.5 && !at_highspeed){
  odrive_serial << "w axis0.controller.config.pos_gain " << 45.0f << '\n';
  odrive_serial << "w axis0.controller.config.vel_gain " << 0.004 << '\n';
  odrive_serial << "w axis0.controller.config.vel_integrator_gain " << 0.00662781f << '\n';


  odrive_serial << "w axis1.controller.config.pos_gain " << 45.0f << '\n';
  odrive_serial << "w axis1.controller.config.vel_gain " << 0.004 << '\n';
  odrive_serial << "w axis1.controller.config.vel_integrator_gain " << 0.00662781f << '\n';
  at_highspeed = true;
}
if(linear < 0.5 && at_highspeed){
  odrive_serial << "w axis0.controller.config.pos_gain " << 45.0f << '\n';
...
ros::Subscriber<geometry_msgs::Twist> update_PID_sub("/encoders/twist", update_PID_params);
