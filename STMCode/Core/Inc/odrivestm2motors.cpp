...
void velCallback(const geometry_msgs::Twist& twist_msg) {
  ...
  if (!wireless_stop)
  {
    SetVelocity(0, left_vel * VEL_TO_RPS);
    SetVelocity(1, left_vel * VEL_TO_RPS);
    SetVelocity(0, right_vel * VEL_TO_RPS);
    SetVelocity(1, right_vel * VEL_TO_RPS);
  }
}
...
void setup() {
  ...
  */
  ...
  run_state(0, requested_state, false);
  HAL_Delay(19000);
  run_state(1, requested_state, false);
  HAL_Delay(19000);
...
void loop() {
  ...
  if (prev_time + pub_period <= current_time)
  {
    ...
    */
    ...
    float left_vel_ = LEFT_POLARITY * GetVelocity(0) / VEL_TO_RPS;
    float right_vel_ = RIGHT_POLARITY * GetVelocity(0) / VEL_TO_RPS;
...
void setUpOdriveParams(UART_HandleTypeDef* odrive_UART_handle)
{

 for (int axis = 0; axis < 2; ++axis) {
    char[100] Data_a, Data_b, Data_c, Data_d, Data_e, Data_f;
    int a, b, c, d, e, f;

    a = sprintf(Data_a, "%s%i%s%i\n", "w axis", axis, ".controller.config.vel_limit", 20);
    HAL_UART_Transmit(odrive_UART_handle, Data_a, a, 250);
    b = sprintf(Data_b, "%s%i%s%d\n", "w axis", axis, ".motor.config.current_lim", 60.0);
    HAL_UART_Transmit(&huart4, Data_b, b, 250);
    c = sprintf(Data_c, "%s%i%s%d\n", "w axis", axis, ".encoder.config.cpr", 42.0);
    HAL_UART_Transmit(&huart4, Data_c, c, 250);
    d = sprintf(Data_d, "%s%i%s%d\n", "w axis", axis, ".controller.config.pos_gain", 45.0);
    HAL_UART_Transmit(&huart4, Data_d, d, 250);
    e = sprintf(Data_e, "%s%i%s%d\n", "w axis", axis, ".controller.config.vel_gain", 0.004);
    HAL_UART_Transmit(&huart4, Data_e, e, 250);
    f = sprintf(Data_f, "%s%i%s%d\n", "w axis", axis, ".controller.config.vel_integrator_gain", 0.00662781);
    HAL_UART_Transmit(&huart4, Data_f, f, 250);
  }
}


void setupODriveParams2(UART_HandleTypeDef* odrive_UART_handle) {
 ...
 for (int axis = 0; axis < 2; ++axis) {
    ...
    a = sprintf(Data_a, "%s%i%s%i\n", "w axis", axis, ".controller.config.vel_limit", 20);
    HAL_UART_Transmit(&huart4, Data_a, a, 250);
    b = sprintf(Data_b, "%s%i%s%d\n", "w axis", axis, ".motor.config.current_lim", 60.0);
    HAL_UART_Transmit(&huart4, Data_b, b, 250);
    c = sprintf(Data_c, "%s%i%s%d\n", "w axis", axis, ".encoder.config.cpr", 42.0);
    HAL_UART_Transmit(&huart4, Data_c, c, 250);
    d = sprintf(Data_d, "%s%i%s%d\n", "w axis", axis, ".controller.config.pos_gain", 45.0);
    HAL_UART_Transmit(&huart4, Data_d, d, 250);
    e = sprintf(Data_e, "%s%i%s%d\n", "w axis", axis, ".controller.config.vel_gain", 0.0132556);
    HAL_UART_Transmit(&huart4, Data_e, e, 250);
    f = sprintf(Data_f, "%s%i%s%d\n", "w axis", axis, ".controller.config.vel_integrator_gain", 0.00662781);
    HAL_UART_Transmit(&huart4, Data_f, f, 250);
  }
...
void closedLoopControl(UART_HandleTypeDef* odrive_UART_handle) {
  int requested_state = 8; //AXIS_STATE_CLOSED_LOOP_CONTROL
