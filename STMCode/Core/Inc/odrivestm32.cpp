...
void update_PID_params(UART_HandleTypeDef * uart_handler, bool *at_highspeed, double linear, double angular, double pos_gain, double vel_gain, double vel_int_gain){

	if(linear > 0.5 && !*at_highspeed){
		set_tuning_parameters(uart_handler, AXIS0, pos_gain, vel_gain, vel_int_gain);
		set_tuning_parameters(uart_handler, AXIS1, pos_gain, vel_gain, vel_int_gain);
		*at_highspeed = true;
	}
	if(linear < 0.5 && at_highspeed){
		set_tuning_parameters(uart_handler, AXIS0, pos_gain, vel_gain, vel_int_gain);
